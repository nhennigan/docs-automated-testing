...
already followed our [FIPS installation guide](fips.md), but stopped after installing
...
so automatically. We recommend following our guide to [configure UFW](/snap/howto/networking/ufw.md). This can
...
DISA STIG host compliance is achieved by running the [USG tool](https://documentation.ubuntu.com/security/docs/compliance/usg/) that is also
...
```sh {"name":"control plane bootstrap"}
...
- {ref}`242410`, {ref}`242411`, {ref}`242412`, and {ref}`242413`: The Kubernetes
   ...
   [ports and services](/snap/reference/ports-and-services/) must be audited in accordance with this list. Those ports,
...
2. Edit
   `/var/snap/k8s/common/etc/configurations/pod-security-admission-baseline.yaml`
   to suit your needs based on the [upstream instructions](https://kubernetes.io/docs/tasks/configure-pod-container/enforce-standards-admission-controller/).
3. Create your own audit policy based on the [upstream instructions](https://kubernetes.io/docs/tasks/configure-pod-container/enforce-standards-admission-controller/) and
   adjust the `--admission-control-config-file` path used in the templates.

For more details, see the [Kubernetes Pod Security Admission documentation](https://kubernetes.io/docs/concepts/security/pod-security-admission/),
...
2. Edit `/var/snap/k8s/common/etc/configurations/audit-policy.yaml` to suit
   your needs based on the [upstream audit instructions](https://kubernetes.io/docs/tasks/debug/debug-cluster/audit/) for this policy file.
3. Create your own audit policy based on the [upstream audit instructions](https://kubernetes.io/docs/tasks/debug/debug-cluster/audit/) and
